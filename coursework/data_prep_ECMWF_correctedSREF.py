'''
==================Stats-CorrectedEnsemble.py=====================
Copy of data_prep_ECMWF_uncorrected, but incorporating UBC-SREF member bias-factor correction
...
fig_dir = '/Users/nadya2/code/coursework/figs_correctedSREF/'
...
data_columns = [3]
time_step = 3
hub_adjustment = 1.36 		#using Log/Power Law
to_kmh = 3.6				#conversion factor to km
mod_num = 18				#number of model members
...
print('---------------------------------------------------')
obs_array = np.empty((stn_cnt,rec_cnt))								#create storage array
...
print('---------------------------------------------------')
ubc_array = np.empty((stn_cnt,mod_num,rec_cnt))								#create storage array
for nStn, stn in enumerate(stn_ids):										#loop through stations
	timeseries = []
...
factor_timeseries = np.empty((stn_cnt,slide_cnt))
...
#calculate correction factor for UBC-SREF
ubc_array_corrected= np.zeros_like(ubc_array) * np.nan
mem_bias_factor = np.zeros((stn_cnt,mod_num))
for nMember in range(mod_num):
	mem_bias_factor[:,nMember] = np.nanmean(obs_array,1)/np.nanmean(ubc_array[:,nMember,:],1)
	for nStn in range(stn_cnt):
		ubc_array_corrected[nStn,nMember,:] = ubc_array[nStn,nMember,:] * mem_bias_factor[nStn,nMember]
ubc_mean_array = np.nanmean(ubc_array_corrected, 1)
...
ensemble_array = np.empty((stn_cnt,mod_num+1,rec_cnt))
ensemble_array[:,:-1,:] = ubc_array_corrected
...
bias_array = np.empty((stn_cnt,3))
mae_array = np.empty((stn_cnt,3))
corr_array = np.empty((stn_cnt,3))
rmse_array = np.empty((stn_cnt,3))
...
np.savetxt(fig_dir + 'Bias.txt', bias_array, fmt='%.2f')
np.savetxt(fig_dir + 'MAE.txt', mae_array, fmt='%.2f')
np.savetxt(fig_dir + 'RMSE.txt', rmse_array, fmt='%.2f')
np.savetxt(fig_dir + 'Corr.txt', corr_array, fmt='%.2f')
...
plt.savefig(fig_dir + 'bias_factor_evolution.pdf', format = 'pdf')
plt.close()
...
plt.savefig(fig_dir + 'timeseriesLL.pdf', format = 'pdf')
plt.close()
...
plt.savefig(fig_dir + 'boxplotsLL.pdf', format = 'pdf')
plt.close()
...
plt.figlegend((l1,l2,l3), ('ECMWF', 'observations', 'UBC-SREF'), loc = 'center right')
plt.savefig(fig_dir + 'timeseriesBF.pdf', format = 'pdf')
plt.close()
...
plt.savefig(fig_dir + 'boxplotsBF.pdf', format = 'pdf')
plt.close()
...
plt.savefig(fig_dir + 'scatterBF.pdf', format = 'pdf')
plt.close()
...
plt.suptitle('Scatter Plot + Linear Regression')
plt.savefig(fig_dir + 'scatterBF_UBC.pdf', format = 'pdf')
plt.close()
...
for nStn, stn in enumerate(stations):
	...
	np.savetxt(fig_dir + 'spectral_obs_%s.csv' %stn, obs_set)
	np.savetxt(fig_dir + 'spectral_ecmwf_%s.csv' %stn, ecmwf_set)



print('---------------------------------------------------')
print('Formatting and saving data for COMPS.......')
...
for nStn, stn in enumerate(stn_ids):
	for nDay, day in enumerate((start_date + datetime.timedelta(n) for n in range(day_cnt))):		#loop through all days and runs
		...
		fcst_save_data = ubc_array_corrected[nStn,:,nDay*smpl_rate:(nDay+1)*smpl_rate]
		...
		filename_fcst = comps_data_dir + 'courseFcstCor/data/'+day.strftime('%Y%m%d')+'_'+str(stn)+'_SFCWSPD.txt'
